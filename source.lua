...
function obfuscate(source,VarName,WaterMark)
        task.spawn(function()
            ...
            local troll_func = [[function() ]]..add_binary(math.random(20,30), "")..[[ end]]
            local troll_var = [[local ]]..Variable..tostring(random_(math.random(15,20)))..[[ = ]]..troll_func

            local SourceByte = ""
            for i = 1,string.len(source) do SourceByte = SourceByte..'"\\'..string.byte(source, i)..'", ' task.wait(.0000001) end
            ...
            local fake_code = function(number,r)
                ...
                for i = 1, tonumber(number) do
                    ...
                    local byte = ""
                    for x = 1,string.len(random) do byte = byte..'"\\'..string.byte(random, x)..'", ' task.wait(.0000001) end
            ...
            warn("Done obfuscate source in "..tostring(tick() - ticks))
        return
    end)
end
